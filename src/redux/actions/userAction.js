...
export const registerUserAction = (registerCredentials) => async (dispatch, getState) => {
  try {
...
export const registerDonor = () => async(dispatch, getState) => {
  ...
  try {console.log('Registering as donor')
    dispatch({ type: REGISTERING_AS_DONOR });
    console.log('locatio +++++'+JSON.stringify(location))
    const res = await registerAsDonor(getAuthToken(token),{latitude:location.latitude,longitude:location.longitude});
